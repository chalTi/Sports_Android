...
public class ImageLoader {
            ...
            if (msg.what == MESSAGE_POST_RESULT) {
                ...
                if (result != null) {
                    ...
                    if (tag.equals(result.url)) {
                        ...
                        Log.i(TAG, "bitmap is setted    " + result.bitmap.toString());
                    } else {
    ...
    private Bitmap getBitmapFromDiskCache(String url, int reqWidth, int reqHeight) throws IOException {
        ...
        if (snapshot != null) {
            ...
            bitmap = ImageUtil.decodeBitmapFromFileDescriptor(fileDescriptor, reqWidth, reqHeight);
            if (bitmap != null) {
     ...
     * @param reqHeight
     */
    public void bindBitmapByUrl(final String url, final ImageView imageView, final int reqWidth, final int reqHeight){
        ...
        Runnable loadTask = new Runnable() {
            @Override
            public void run() {
                Bitmap bitmap = loadBitmap(url, imageSize.width, imageSize.height);
                if (bitmap != null) {
     ...
     * @param imageView
     */
    public void bindBitmapByPath(final String path, final ImageView imageView){
        ...
        Runnable loadTask = new Runnable() {
            @Override
            public void run() {
                Bitmap bitmap = loadBitmapByPath(path, imageSize.width, imageSize.height);
                if (bitmap != null) {
                    LoaderResult result = new LoaderResult(imageView, path, bitmap);
                    Message message = mUiHandler.obtainMessage();
                    message.obj = result;
                    message.what = MESSAGE_POST_RESULT;
                    mUiHandler.sendMessage(message);
     ...
     * @return
     */
    private Bitmap loadBitmapByPath(String path, int reqWidth, int reqHeight) {
        //先看缓存中有没有，有的话就直接用
        Bitmap bitmap = getBitmapFromCache(path);
        if (bitmap != null) {
            return bitmap;
        ...
        //没有就从本地中读取出来
        bitmap = ImageUtil.decodeBitmapFromPath(path, reqWidth, reqHeight);
        if (bitmap != null) {
